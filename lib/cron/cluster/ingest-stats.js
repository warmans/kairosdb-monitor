...
function updateIngestStats(successCallback){
    ...
    global.memoryCache.get('cluster.status', function (err, result) {
        ...
        if (!payload.query_node) {
            //no valid node was found
            return;
        }

        var options = {
            uri: 'http://'+payload.query_node.hostname+':'+payload.query_node.port+'/api/v1/datapoints/query',
            method: 'POST',
        ...
        request(options, function(error, response, body) {
            ...
            if (error) {
                ...
                if (response.statusCode === 200) {
                    ...
                    payload.success = false;
                    payload.msg = 'Unexpected response status: '+response.statusCode;
                }
            }
