    ...
    function getNodeStatus(){
        ...
        request(options, function(error, response, body) {
            ...
            if (!error && response.statusCode === 200) {
                hostStatus.status = hostStatus.time > 1000 ? 'US' : 'UN';
...
module.exports.getStatus = function() {
    ...
    for (var hostKey in (config.kairos_hosts ? config.kairos_hosts : [])) {

        //history is in reverse order
        var statusHistory = status_history[hostKey] ? status_history[hostKey] : [];
        ...
        statusHistory.forEach(function(curStatus) {
            ...
            if (curStatus['status'] !== 'DN') {
                ...
                //history is in reverse order so flip it back to be left-to-right
                status.nodes[hostKey]['response_time_ms_history'].unshift(rt);
            }
        });
    }
